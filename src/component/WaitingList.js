...
export default function WaitingList() {
    ...
    useEffect(() => {
        fetch(`${baseURL}/api/getWaitingDetails`)
...
<ul className="nav nav-tabs">
  <li className="nav-item">
    <Link className="nav-link active" aria-current="page" to="/WaitingList">Patient Wise</Link>
  </li>
  <li className="nav-item">
    <Link className="nav-link" to="/TestWaitingList">Test Wise</Link>
  </li>
</ul> 
</nav>
        ...
        <table className='table  table-nowrap mb-0 scrollable-table mt-3'>
          ...
          {data.map((item, index) => (
              <tr key={index} className='text-center'>
                ...
                <td><span className='badge badge-danger-lighten'>{item.Pending}</span></td>
                <td><span className='badge badge-info-lighten'>{item.Complete}</span></td>
                <td><span className='badge badge-success-lighten'>{item.Approved}</span></td>
