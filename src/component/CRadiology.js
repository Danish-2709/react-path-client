...
export default function CRadiology() {
    const { TID } = useParams();
    ...
    const [tType, setTType] = useState('RADIOLOGY');
...
const fetchOptions = async () => {
    try {
        const response = await fetch(`${baseURL}/api/getRdata`); 
        if (response.ok) {
...
console.log('Component rendered with selectedTGNameId:', selectedTGNameId);
    ...
    if(TID){
        try {
          const response = await fetch(`${baseURL}/api/UpdateCTest/${TID}`, {
            ...
            body: JSON.stringify({ TID, selectedTGNameId, tName, tCheck, isHeading, pSL, mName, uName, tType, dValue }),
          });
      
          if (response.ok) {
            console.log('Radiology Test Updated successfully');
            setSuccessMessage('Radiology Test Updated successfully.');
            setSelectedOption('');
            setTName('');
            setTCheck('');
            setISHeading('');
      ...
      try {
        const response = await fetch(`${baseURL}/api/CTest`, {
          ...
          body: JSON.stringify({ selectedTGNameId, tName, tCheck, isHeading, pSL, mName, uName, tType, dValue }),
        });
    
        if (response.ok) {
          console.log('Radiology Test Created successfully');
          setSuccessMessage('Radiology Test Created successfully.');
          setSelectedOption('');
          setTName('');
          setTCheck('');
          setISHeading('');
...
<div className='container mt-3 '>
  <div className='shadow-sm p-2 mb-2 bg-body-tertiary rounded'>
    <h6 className='text-center c2'>{TID ? 'Update Radiology Test' : 'Create Radiology Test'}</h6>
           ...
           <div className='row'>
               ...
               <div>
                    ...
                    <ReactQuill modules={{ toolbar: [ [{ 'header': '1' }, { 'header': '2' }, { 'font': [] }],
                                ...
                                [{ 'background': [] }, { 'color': [] }],], }} value={dValue}  onChange={handleDValueChange} />
                    </div>
               </div>
            </div>
