...
export default function CTNRange() {
  const { TID } = useParams();
  ...
  const [tType, setTType] = useState('NORMAL RANGE');
  ...
  const handleSubmit = async (e) => {
    ...
    if(TID){
      try {
        const response = await fetch(`${baseURL}/api/UpdateCTest/${TID}`, {
          ...
          body: JSON.stringify({TID, selectedTGNameId, tName, tCheck, isHeading, pSL, mName, uName, tType, dValue, mMVal, mFVal, maxMVal, maxFVal, mMWarn, mFWarn, maxMWarn, maxFWarn, dTM, dTF}),
        });
    
        if (response.ok) {
          console.log('Normal Test Updated successfully');
          setSuccessMessage('Normal Test Updated successfully.');
          setSelectedOption('');
          setTName('');
          setTCheck('');
          setISHeading('');
    ...
    try {
      const response = await fetch(`${baseURL}/api/CTest`, {
        ...
        body: JSON.stringify({ selectedTGNameId, tName, tCheck, isHeading, pSL, mName, uName, tType, dValue, mMVal, mFVal, maxMVal, maxFVal, mMWarn, mFWarn, maxMWarn, maxFWarn, dTM, dTF}),
      });
      if (response.ok) {
        console.log('Normal Test Created successfully');
        setSuccessMessage('Normal Test Created successfully.');
        setSelectedOption('');
        setTName('');
        setTCheck('');
        setISHeading('');
      ...
      fetch(`${baseURL}/api/EditTest/${TID}`)
        ...
        .then((result) => {
          ...
          if (result && result.length > 0) {
            ...
            setMMVal(data.MinVM || '');
            setMFVal(data.MinV || '');
            setMaxMVal(data.MaxVM || '');
            setMaxFVal(data.MaxV || '');
            setMMWarn(data.MinWM || '');
            setMFWarn(data.MinW || '');
            setMaxMWarn(data.MaxWM || '');
            setMaxFWarn(data.MaxW || '');
...
{showModal && (
    ...
    <div className={`modal fade ${showModal ? 'show' : ''}`} style={{ display: showModal ? 'block' : 'none' }} tabIndex="-1" role="dialog">
     ...
     <div className='shadow-sm p-2 mb-2 bg-body-tertiary rounded'>
        <h6 className='text-center c2'>{TID ? 'Update Test Normal Range' : 'Create Test Normal Range'}</h6>
